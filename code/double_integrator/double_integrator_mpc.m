function double_integrator_mpc(xInit, save_file, sim_time)
  ...
  for k = 1:N
    ...
    dx = xk(1:2) - xkr;
    du = uk;
    cost = cost + dx'*Q*dx + du'*R*du +1e12*uk(2);
  ...
  % Terminal set with relaxation
  g = [g; -K*(xN(1:2)-xrN) - V(iV('u',N,'s2'))];
  iG_.obs{N+1} = 1 + offset;
  offset = offset + 1;
  g = [g; -K*(xN(1:2)-xrN) + V(iV('u',N,'s2'))];
  iG_.obs{N+2} = 1 + offset;
  offset = offset + 1;
  cost = cost + 1000*V(iV('u',N,'s2'));
  ...
  ubg(iG_.obs{N+1}) = a_max; lbg(iG_.obs{N+1}) = -inf;
  lbg(iG_.obs{N+2}) = a_min; ubg(iG_.obs{N+2}) = inf;
  ...
  lbv(iV('u',1:N,'s2')) =  zeros(size(iV('u',1:N,'s2')));
  ...
  save(save_file,'X','U','ref','tx','tu','Q','R',...
                 'XP_open','XV_open','UV_open','ts','time_array');
end